...
If a single share was bought on day P and sold on day Q, where 0 ≤ P ≤ Q < N, then the profit of such
...
N consecutive days, returns the maximum possible profit from one transaction during this period.
...
def solution(A):
    max_profit = 0
    bottom_idx = upper_idx = 0

    if len(A) < 2:
        return 0

    for index, stock_price in enumerate(A):
        if A[bottom_idx] > stock_price:
            bottom_idx = index

        if A[bottom_idx] < stock_price:
            upper_idx = index

        if upper_idx > bottom_idx and max_profit < A[upper_idx] - A[bottom_idx]:
            max_profit = A[upper_idx] - A[bottom_idx]
